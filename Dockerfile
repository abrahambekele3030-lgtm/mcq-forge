...
# The start command runs `prisma db push` to create/migrate the schema, then
# launches Next.js. This ensures tables exist on first deploy and after any
# schema change.
CMD ["sh", "-c", "bunx prisma db push --accept-data-loss && exec bun run start"]
